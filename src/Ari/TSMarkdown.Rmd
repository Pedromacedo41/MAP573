...
```{r ACF}
library(forecast)
p1 <- ggAcf(TS_sum[1:48],) + labs(title="Autocorrelations for lags 48 hours")+theme_bw()
p2 <- ggAcf(TS_sum[1:840], lag.max = 24*35) + labs(title="Autocorrelations for lags 840 hours (35 days)")+theme_bw()
grid.arrange(p1, p2, nrow = 2)
```
...
```{r PACF}
p1 <- ggAcf(TS_sum[1:24], type = "partial")+theme_bw()+labs(title="PACF for 24 hours")
p2 <- ggAcf(TS_sum[1:(24*7)], type = "partial", lag.max = 24*7)+theme_bw()+labs(title="PACF for a week")
p3 <- ggAcf(TS_sum[1:(24*7*4)], type = "partial", lag.max = 24*7*4)+theme_bw()+labs(title="PACF for a month")
